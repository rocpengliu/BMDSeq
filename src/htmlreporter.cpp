...
void HtmlReporter::setKmer(std::map<uint32_t, int>& kmerCountFreq, uint32_t kmerCount){
    mKmerCountFreq = kmerCountFreq;
    mKmerCount = kmerCount;
    //mFreqMap = freqMap;
}
...
void HtmlReporter::printSummary(ofstream& ofs, FilterResult* result, Stats* preStats1, Stats* postStats1, Stats* preStats2, Stats* postStats2) {
    ...
    ofs << endl;
    ofs << "<h1 style='text-align:left;'><a href='https://github.com/rocpengliu/Seq2Kmer' target='_blank' style='color:#006400;text-decoration:none;'>" + mOptions->reportTitle + "</a>"<<endl;
    ...
    outputRow(ofs, "seq2kmer version:", string(SEQ2KMER_VER)+ " (<a href='https://github.com/rocpengliu/Seq2Kmer'>https://github.com/rocpengliu/Seq2Kmer</a>)");
    ...
    ofs << "<div class='subsection_title' onclick=showOrHide('kmer_summary')>Kmer summary</div>\n";
    ofs << "<div id='kmer_summary'>\n";
    ofs << "<table class='summary_table'>\n";
    if(!mKmerCountFreq.empty()){
        outputRow(ofs, "kmer size:", mOptions->kmerSize);
        outputRow(ofs, "n. of kmers:", formatNumber(mKmerCount));
        outputRow(ofs, "max. kmer:", (*mKmerCountFreq.rbegin()).first);
        outputRow(ofs, "min. kmer:", (*mKmerCountFreq.begin()).first);
    }
    ofs << "</table>\n";
    ofs << "</div>\n";
    ...
    ofs << "</div>\n";
     ...
     if(!mKmerCountFreq.empty()) {
        ofs << "<div class='section_div'>\n";
        ofs << "<div class='section_title' onclick=showOrHide('kmers')><a name='summary'>Kmers</a></div>\n";
        ofs << "<div id='kmers'>\n";
        reportKmers(ofs);
        ofs << "</div>\n";
        ofs << "</div>\n";
...
void HtmlReporter::reportKmers(ofstream& ofs){
    ofs << "<div id='kmer_freq_figure'>\n";
    ofs << "<div class='figure' id='plot_kmer_freq' style='height:400px;'></div>\n";
    ofs << "</div>\n";

    ofs << "<div class='sub_section_tips'>There are ";
    ofs << to_string(mKmerCount) << " " << to_string(mOptions->kmerSize);
    ofs << "-kmers, and the histograms are based on 100";
    ofs <<"</div>\n";

    ofs << "\n<script type=\"text/javascript\">" << endl;
    auto itp = Stats::list2string(mKmerCountFreq, mKmerCount);
    string json_str = "var data=[";
    json_str += "{";
    json_str += "x:[" + itp.first + "],";
    json_str += "y:[" + itp.second + "],";
    json_str += "name: 'Count  ',";
    json_str += "type:'bar',";
    json_str += "line:{color:'rgba(128,0,128,1.0)', width:1}\n";
    json_str += "}";
    json_str += "];\n";
    json_str += "var layout={title:'Kmer (" + std::to_string(mOptions->kmerSize) +  " nt) count frequency', xaxis:{title:'Kmer count'}, yaxis:{title:'Kmer frequency(%)'}};\n";
    json_str += "Plotly.newPlot('plot_kmer_freq', data, layout);\n";
...
void HtmlReporter::printHeader(ofstream& ofs){
    ...
    ofs << "<title>Seq2Kmer report at " + getCurrentSystemTime() + " </title>";
    printJS(ofs);
    printCSS(ofs);
    ofs << "</head>";
...
void HtmlReporter::printCSS(ofstream& ofs){
    ...
    ofs << "#menu a {color:#ADD8E6; font-size:18px;font-weight:600;line-height:28px;text-decoration:none;font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'}" << endl;
    ...
    ofs << ".section_title {color:#ffffff;font-size:20px;padding:5px;text-align:left;background:#006400; margin-top:10px;}" << endl;
    ofs << ".subsection_title {font-size:16px;padding:5px;margin-top:10px;text-align:left;color:#006400}" << endl;
    ...
    ofs << "#footer {text-align:left;padding:15px;color:#ffffff;font-size:10px;background:#006400;font-family:Arail,'Liberation Mono', Menlo, Courier, monospace;}" << endl;
...
void HtmlReporter::printFooter(ofstream& ofs){
    ...
    ofs << "<div id='footer'> ";
    ofs << "<p>"<<command<<"</p>";
    ofs << "seq2kmer " << SEQ2KMER_VER << ", at " << getCurrentSystemTime() << " </div>";
    ofs << "</body></html>";
}
