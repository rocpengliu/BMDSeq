...
void PairEndProcessor::initOutput() {
    ...
    // if(!mOptions->kmerOut.empty()){
    //     mKmerOutWriter = new WriterThread(mOptions, mOptions->kmerOut);
    // }
}
...
bool PairEndProcessor::process(){
    ...
    std::thread* kmerWriterThread = new std::thread(std::bind(&PairEndProcessor::writeKmerOutput, this));
        ...
        if(mOptions->merge.enabled && r1 && r2) {
            ...
            if(ov.overlapped) {
                ...
                if(result == PASS_FILTER) {
                    if(merged->length() >= mOptions->kmerSize){
                        if(mOptions->kmerSize > 32){
                            Kmer::processKmerMapPair(merged, subKmerMap128, mOptions->kmerSize, mOptions->NoRevKmer, mOptions->complexityFilter.threshold, mOptions->complexityFilter.enabled);
                        } else {
                            Kmer::processKmerMapPair(merged, subKmerMap, mOptions->kmerSize, mOptions->NoRevKmer, mOptions->complexityFilter.threshold, mOptions->complexityFilter.enabled);
                        }
                    }
                ...
                if(result2 == PASS_FILTER){
                    if(r2->length() >= mOptions->kmerSize){
                        if(mOptions->kmerSize > 32){
                            Kmer::processKmerMapPair(r2, subKmerMap128, mOptions->kmerSize, mOptions->NoRevKmer, mOptions->complexityFilter.threshold, mOptions->complexityFilter.enabled);
                        } else {
                            Kmer::processKmerMapPair(r2, subKmerMap, mOptions->kmerSize, mOptions->NoRevKmer, mOptions->complexityFilter.threshold, mOptions->complexityFilter.enabled);
                        }
                    }
                }
...
void PairEndProcessor::addKmerMap(std::unordered_map<uint64_t, uint32_t>& subKmerMap){
    kmermtx.lock();
...
void PairEndProcessor::addKmerMap(std::unordered_map<uint128_t, uint32_t, uint128_hash>& subKmerMap128){
    kmermtx.lock();
...
void PairEndProcessor::processorTask(ThreadConfig* config)
{
    ...
    if(mFinishedThreads == mOptions->thread) {
        ...
        // if(mKmerOutWriter)
        //     mKmerOutWriter->setInputCompleted();
    }
...
void PairEndProcessor::writeKmerOutput(){
    if(mOptions->verbose){
        loginfo("starting to write kmer table!");
    }
    ...
    if(mOptions->kmerSize > 32){
        for(const auto & it : *kmerMap128){
            ...
            if (it.second < mOptions->kmerFilter)
                continue;
            //oss << Kmer::uint2seq(it.first, mOptions->kmerSize) << "\t" << it.second << "\t" << Kmer::print_uint_binary(it.first, mOptions->kmerSize) << "\n";
            oss << Kmer::uint2seq(it.first, mOptions->kmerSize) << "\t" << it.second << "\n";
            //oss << Kmer::uint128_to_string(it.first) << "\t" << it.second << "\n";
            if(count % 1000000 == 0 && count > 0){
                loginfo("write " + std::to_string(count / 1000000) + "M lines for kmer output file!");
            }
