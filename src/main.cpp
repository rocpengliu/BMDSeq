...
int main(int argc, char* argv[]){
    ...
    if(argc == 1) {
        cerr << "seq2kmer: an ultra-fast all-in-one kmer extractor" << endl << "version " << SEQ2KMER_VER << endl;
    }
    ...
    if (argc == 2 && (strcmp(argv[1], "-v")==0 || strcmp(argv[1], "--version")==0)){
        cerr << "seq2kmer " << SEQ2KMER_VER << endl;
        return 0;
    }
    cmdline::parser cmd;
    // input/output
    ...
    cmd.add<int>("kmer_size", 0, "kmer size must be between 5 and 64, default: 31", false, 31);
    cmd.add("no_rev_kmer", 0, "extract kmer from both forward and reverse read, default: enable, if specified: disenable");
    cmd.add<string>("kmer_out", 0, "kmer output file", false, "");
    ...
    cmd.add<int>("umi_skip", 0, "if the UMI is in read1/read2, seq2kmer can skip several bases following UMI, default is 0", false, 0);
    ...
    // reporting
    cmd.add<string>("json", 'j', "the json format report file name", false, "seq2kmer.json");
    cmd.add<string>("html", 'h', "the html format report file name", false, "seq2kmer.html");
    cmd.add<string>("report_title", 'R', "should be quoted with \' or \", default is \"Seq2Kmer report\"", false, "Seq2Kmer report");
    ...
    opt->NoRevKmer = cmd.exist("no_rev_kmer");
    opt->kmerOut = cmd.get<string>("kmer_out");
    ...
    if(opt)
        delete opt;
    time_t t2 = time(NULL);
    cerr << "seq2kmer v" << SEQ2KMER_VER << ", time used: " << (t2)-t1 << " seconds" << endl;
