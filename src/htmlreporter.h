...
public:
    ...
    void setKmer(std::map<uint32_t, int>& kmerCountFreq, uint32_t kmerCount);
...
private:
    ...
    int mInsertSizePeak;
    std::map<uint32_t, int> mKmerCountFreq;
    uint32_t mKmerCount;
    //std::map<int, uint64_t> mFreqMap;
};
