...
bool Options::validate() {
    ...
    if(kmerSize < 5 || kmerSize > 64){
        error_exit("kmer size must be between 5 and 64!");
    }

    if(kmerOut.empty()){
        error_exit("kmer output file is not specified by <ker_out>");
    } else {
        if(!ends_with(kmerOut, ".gz")){
            kmerOut.append(".gz");
        }
    }
    ...
    if(merge.enabled) {
        ...
        if(merge.out.empty() && !outputToSTDOUT && !out1.empty() && out2.empty()) {
            cerr << "You specified --out1, but haven't specified --merged_out in merging mode. Using --out1 to store the merged reads to be compatible with seq2kmer 0.19.8" << endl << endl;
            merge.out = out1;
            out1 = "";
        }
    ...
    if(thread < 1) {
        ...
        cerr << "WARNING: seq2kmer uses up to 16 threads although you specified " << thread << endl;
        thread = 16;
    }
...
void Options::parseSample(){
    if (in_dir.empty())
        error_exit("sample directory is not specified");
    if(!is_directory(in_dir)) error_exit(in_dir + " sample directory does not exist");
    DIR* dir = opendir(in_dir.c_str());
    std::string pattern = "_kmer_"+ std::to_string(kmerSize) + ".tab.gz"; // Change the pattern as needed
    struct dirent* entry;
    while ((entry = readdir(dir)) != nullptr) {
        std::string filename = entry->d_name;
        if (filename != "." && filename != ".." && filename.find(pattern) != std::string::npos) {
            samples.push_back(in_dir + "/" + filename);
        }
    }
    closedir(dir);
    if(samples.empty()) error_exit("no sample file *tab.gz found!");
}

void Options::readMeta(){
    loginfo("starting to read meta table!");
	ifstream infile;
	infile.open(metatable.c_str(), ifstream::in);
	const int maxLine = 1000;
	char line[maxLine];
	std::vector<std::string> spltVec;
    std::multimap<double, std::string> meta_mmap;
	while(infile.getline(line, maxLine)){
		...
		spltVec.clear();
		::split(linestr, spltVec, "\t");
		if(spltVec.size() != 2){
			continue;
		}
        double dose = std::stod(spltVec[1]);
		meta_mmap.insert({dose, spltVec[0]});
		bmd.doseVec.emplace_back(dose);
	}

    std::sort(bmd.doseVec.begin(), bmd.doseVec.end());
	bmd.numDose = std::unordered_set<int>(bmd.doseVec.begin(), bmd.doseVec.end()).size();
    int index = 0;
    std::vector<std::string> sam_vec;
	for(auto it = meta_mmap.begin(); it != meta_mmap.end();){
        sam_vec.clear();
		auto range = meta_mmap.equal_range(it->first);
		for(auto itr = range.first; itr != range.second; ++itr){
			sam_vec.emplace_back(itr->second);
		}
		std::sort(sam_vec.begin(), sam_vec.end());
        bmd.doseMap[it->first] = std::make_pair(sam_vec, bmd.samMap.size());
		for(auto itv : sam_vec){
			bmd.samMap[itv] = index;
            ++index;
            bmd.doseRankVec.emplace_back(index);
		}
        it = range.second;
	}
    if(bmd.doseMap.empty()) error_exit("no meta data found!");
    loginfo("reading meta table with size of " + std::to_string(bmd.samMap.size()) + " done!");
}