...
std::pair<string, string> Stats::list2string(std::map<uint32_t, int>& freqMap, uint32_t& totCount){
    stringstream ssx;
    stringstream ssy;
    int i = 0;
    for(auto it = freqMap.begin(); it != freqMap.end(); ++it){
        if(i == 100)
            break;
        ssx << it->first << (std::next(it) == freqMap.end() ? "" : ",");
        ssy << getPer(it->second, totCount) << (std::next(it) == freqMap.end() ? "" : ",");
        ++i;
    }
    return std::make_pair(ssx.str(), ssy.str());
}
