...
class Bmd{
    public:
        Bmd(){
            numDose = 0;
            doseVec.clear();
            doseRankVec.clear();//1,2,3,4, as.factor in R
            doseMap.clear();
            kmerMap.clear();
            samMap.clear();
            kmerMap128.clear();
    ...
    public:
        int numDose;
        std::vector<double> doseVec;
        std::vector<int> doseRankVec;
        std::map<double, std::pair<std::vector<std::string>, int>> doseMap;//dose, samples.index
        std::unordered_map<std::string, int> samMap;
        std::unordered_map<uint64_t, std::vector<uint32_t>> kmerMap;
        std::unordered_map<uint128_t, std::vector<uint32_t>, uint128_hash> kmerMap128;
};
