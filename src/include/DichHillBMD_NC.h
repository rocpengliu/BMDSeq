...
#  define HILL_MEAN(G, N, A, B, D) \
    ((D) <= (0.0)) ? (G) : (G + (1 - G) * N / (1 + exp(-A - B * log(D))))
 ...
 public:
  ...
  std::vector<double> fixConstraintUB(
    ...
    if (isExtra) {
      ...
      temp = (Z - ub * log(BMD)) *
             1.1;  // add a little extra add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0);
      x[2] = theta(2, 0) - temp;
    } else {
  ...
  std::vector<double> fixConstraintLB(
    ...
    if (isExtra) {
      Z = HILL_EXTRA_Z(g, n, a, NULL, BMR);
      temp = (lb * log(BMD) - Z) *
             2;  // add a little extra add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0);
      x[2] = theta(2, 0) - temp;
    } else {
  ...
  Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd X) {
    ...
    double a = HILL_A(theta(2, 0));
    double b = HILL_B(theta(3, 0));

    Eigen::MatrixXd p(X.rows(), 1);

    for (int i = 0; i < X.rows(); i++) p(i, 0) = HILL_MEAN(g, n, a, b, X(i, 2));
  ...
  double compute_BMD_EXTRA_NC_INEQUALITY(
    ...
    if (grad) {
      grad[0] = 0.0;  // gamma
      grad[1] = -1.0 * (n / BMR) * exp(theta(1, 0)) /
                ((exp(theta(1, 0)) + 1) * ((n / BMR - 1.0) * exp(theta(1, 0)) - 1));
      grad[2] = -1.0;  // alpha
    }
  ...
  double compute_BMD_ADDED_NC_INEQUALITY(
    ...
    if (grad) {
      grad[0] = -(n / (n * g - n + BMR)) *
                (exp(theta(1, 0)) / ((exp(theta(1, 0) + 1) * (exp(theta(1, 0) + 1)))));  // gamma
      grad[1] = (g - 1) / ((g - 1) * n + BMR) *
                (exp(theta(1, 0)) / ((exp(theta(1, 0) + 1) * (exp(theta(1, 0) + 1)))));
      grad[2] = -1.0;  // alpha
    }
