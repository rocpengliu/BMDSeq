...
class normalLLTESTA1 : public normalLL {
 public:
  normalLLTESTA1() {};
  normalLLTESTA1(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS) : normalLL(tY, tX, SS) {
  ...
  int nParms() { return meanX.cols() + 1; };
  ...
  Eigen::MatrixXd variance(Eigen::MatrixXd theta) { return variance(theta, X); };
...
class normalLLTESTA2 : public normalLL {
 public:
  normalLLTESTA2() {};
  normalLLTESTA2(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS) : normalLL(tY, tX, SS) {
    // if it is a sufficient statistics model
    std::vector<double> vec(tX.data(), tX.data() + tX.rows() * tX.cols());
    std::sort(vec.begin(), vec.end());
    vec.erase(std::unique(vec.begin(), vec.end()), vec.end());
  ...
  int nParms() { return 2 * meanX.cols(); };
...
class normalLLTESTA3 : public normalLL {
 public:
  normalLLTESTA3() {};
  normalLLTESTA3(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS) : normalLL(tY, tX, SS) {
    // if it is a sufficient statistics model
    std::vector<double> vec(tX.data(), tX.data() + tX.rows() * tX.cols());
    std::sort(vec.begin(), vec.end());
    vec.erase(std::unique(vec.begin(), vec.end()), vec.end());
  ...
  // mean and variance for every dose group
  int nParms() { return meanX.cols() + 2; };
  ...
  Eigen::MatrixXd variance(Eigen::MatrixXd theta, Eigen::MatrixXd d) {
    ...
    Eigen::MatrixXd rV =
        exp(theta(theta.rows() - 1, 0)) *
        pow(abs(tmean.array()), theta(theta.rows() - 2, 0));  //  theta(theta.rows() - 2, 0));
    return rV;
  };
...
class normalLLTESTR : public normalLL {
 public:
  normalLLTESTR() {};
  normalLLTESTR(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS)
      : normalLL(tY, tX, SS) {
