...
class lognormalEXPONENTIAL_BMD_NC : public lognormalLLModel {
 public:
  lognormalEXPONENTIAL_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS, int degree)
      : lognormalLLModel(tY, tX, SS) {
    // if it is a sufficient statistics model
    deg = degree;
  };

  lognormalEXPONENTIAL_BMD_NC(Eigen::MatrixXd tY, Eigen::MatrixXd tX, bool SS, bool CV, int degree)
      : lognormalLLModel(tY, tX, SS) {
    // if it is a sufficient statistics model
    deg = degree;
  };
  ...
  virtual Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd d) {
    ...
    switch (deg) {
      ...
      case LOGNORMAL_EXP4_UP:
        rV = theta(0, 0) *
             (exp(theta(2, 0)) - (exp(theta(2, 0)) - 1.0) * exp(-theta(1, 0) * d.array()));
        break;
      case LOGNORMAL_EXP5_DOWN:
      case LOGNORMAL_EXP5_UP:
      default:
        rV = theta(0, 0) * (exp(theta(2, 0)) - (exp(theta(2, 0)) - 1.0) *
                                                   exp(-pow(theta(1, 0) * d.array(), theta(3, 0))));
    }
