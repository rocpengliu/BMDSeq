...
#define LOGPROBIT_MEAN(G, A, B, D) \
  ((D) <= (0.0)) ? (G) : (G + (1 - G) * gsl_cdf_gaussian_P(A + B * log(D), 1))
...
class dich_logProbitModelNC : public binomialBMD {
 public:
  ...
  Eigen::MatrixXd mean(Eigen::MatrixXd theta, Eigen::MatrixXd X) {
    ...
    for (int i = 0; i < X.rows(); i++) p(i, 0) = LOGPROBIT_MEAN(g, a, b, X(i, 2));
  ...
  std::vector<double> fixConstraintLB(
    ...
    if (isExtra) {
      ...
      temp = (lb * log(BMD) - Z) *
             1.2;  // add a little extra add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0) - temp;
    } else {
  ...
  std::vector<double> fixConstraintUB(
    ...
    if (isExtra) {
      Z = LOGPROBIT_EXTRA_Z(g, a, BMR);
      temp = (Z - ub * log(BMD)) *
             1.01;  // add a little extra add a little extra so the constraint is satisfied
      x[0] = theta(0, 0);
      x[1] = theta(1, 0) - temp;
    } else {
      Z = LOGPROBIT_ADDED_Z(g, a, BMR);
