// File:    IDPriorMCMC.h
// Purpose: Sets the priors for the binomial model  and continuous model parameters these methods
// are general
...
//                      basic definition to be used across types of models (continuous/dichotomous
//                      etc)
//
//
//
...
class IDPriorMCMC {
 public:
  IDPriorMCMC(IDPriorMCMC &M) { prior_spec = M.prior_spec; }

  // Binomial Prior Coinstructor
  IDPriorMCMC(Eigen::MatrixXd tP)
      : prior_spec(tP) {
  ...
  void set_prior(Eigen::MatrixXd tP) { prior_spec = tP; }
  double neg_log_prior(Eigen::MatrixXd theta);
  Eigen::MatrixXd log_prior(Eigen::MatrixXd theta);
  Eigen::MatrixXd prior_mean();

  Eigen::MatrixXd lowerBounds() { return prior_spec.col(3); }

  Eigen::MatrixXd upperBounds() { return prior_spec.col(4); }
    ...
    if (parm >= 0 && parm < prior_spec.rows()) {
      switch (prior_iidtype(prior_spec(parm, 0))) {
        case prior_iidtype::iid_normal:
          ...
          prior_spec(parm, 2) *=
              fabs(scale);  // note it is only scale because we are dealing with the SD
  ...
  Eigen::MatrixXd get_prior() { return prior_spec; }
